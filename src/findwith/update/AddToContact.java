...
/**
 * Created by milinchuk on 12/10/14.
 */
public class AddToContact extends HttpServlet {
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        if( request.getSession().getAttribute("login") != null){
            ...
            if(!contacts.contains(request.getParameter("contact").toString())){
                ...
                ((Person) request.getSession().getAttribute("person")).getContacts().add(request.getParameter("contact").toString());
